...
// Add this to Fov-Charfix.js after the console.log('Force Default Skin'); line

// This modifies the setWeapon function to adjust weapon position
pc.Application.getApplication().once("postrender", function () {
    // Store the original setWeapon function
    const originalSetWeapon = pc.app.scripts.list()[11].prototype.setWeapon;
    
    // Override the setWeapon function
    pc.app.scripts.list()[11].prototype.setWeapon = function(weaponName) {
        // Call the original function first
        originalSetWeapon.call(this, weaponName);
        
        // After the weapon is set, adjust its position if it's AR or Scar
        if (weaponName && (weaponName.includes('AR') || weaponName.includes('Scar'))) {
            console.log('Adjusting position for ' + weaponName);
            
            // Wait a short time to ensure the weapon is loaded
            setTimeout(() => {
                if (this.weaponHolder) {
                    // Get current local position
                    const currentPos = this.weaponHolder.getLocalPosition();
                    
                    // Create a new position that's farther away
                    // Adjust these values to move the weapon:
                    // - z: negative values move the weapon forward (away from player)
                    // - y: positive values move the weapon up
                    // - x: positive values move the weapon to the right
                    this.weaponHolder.setLocalPosition(
                        currentPos.x,     // Keep the same x position
                        currentPos.y,     // Keep the same y position 
                        currentPos.z - 0.5 // Move weapon 0.5 units farther away
                    );
                    
                    console.log('Weapon position adjusted for ' + weaponName);
                }
            }, 100);
        }
    };
    
    console.log('Weapon position adjustment enabled');
});
